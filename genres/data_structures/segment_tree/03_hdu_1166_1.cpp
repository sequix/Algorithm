// NDU No.1755 I Hate It (range query & point update) (390ms 3256kB)
#include <cstdio>
using namespace std;
...
int query(int o, int L, int R)
{
    if(L <= nd[o].L && nd[o].R <= R) {
        ...
        int sum = 0;
        int M = (nd[o].L + nd[o].R) >> 1;
        if(R <= M) sum = query(lc(o), L, R);
        else if(L > M) sum = query(rc(o), L, R);
        else sum = query(lc(o), L, M) + query(rc(o), M+1, R);
        return sum;
    }
}
